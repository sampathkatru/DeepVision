...
CONFIG_PATH = BASE_DIR / "configs" / "g_model_config.json"
MODEL_PATH  = BASE_DIR / "models" / "glaucoma_model.pth"

with open(CONFIG_PATH) as f:
    config = json.load(f)

CLASS_NAMES   = config.get("class_names", ["Normal", "Glaucoma"])
...
def _get_model():
    global _model
    if _model is None:
        _model = GlaucomaClassifier(num_classes=len(CLASS_NAMES))
        state = torch.load(MODEL_PATH, map_location="cpu")
        if isinstance(state, dict) and "state_dict" in state:
            state = state["state_dict"]
        if isinstance(state, GlaucomaClassifier):
            _model = state
        else:
...
def predict_glaucoma(pil_image):
    model = _get_model()
